...
aud_list = [
    "./ArrayCore/mAudio/AUD1.mp3",
    "./ArrayCore/mAudio/AUD2.mp3",
]
...
async def vcraid(_, e: Message):
    ...
    if inp:
        ...
        if chat_id in QUEUE:
            ...
            await e.reply_text(f"**> Raiding porm in:** {chat_.title} \n\n**> Audio:** {songname} \n**> Position:** #{pos}")
        else:
            if call_py1:
...
@vcbot.on_message(filters.user(SUDO_USERS) & filters.command(["endm"], prefixes=HNDLR))
async def ping(_, e: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            clear_queue(chat_id)
            await e.reply_text("**Porn Raid Ended!**")
...
@vcbot.on_message(filters.user(SUDO_USERS) & filters.command(["mpause"], prefixes=HNDLR))
async def ping(_, e: Message):
    gid = e.chat.id
    uid = e.from_user.id
    if gid == uid:
    ...
    if chat_id in QUEUE:
        try:
            ...
            await e.reply_text(f"**Porn raid Paued In:** {chat_.title}")
...
@vcbot.on_message(filters.user(SUDO_USERS) & filters.command(["mresume"], prefixes=HNDLR))
async def ping(_, e: Message):
    gid = e.chat.id
    uid = e.from_user.id
    if gid == uid:
    ...
    if chat_id in QUEUE:
        try:
            ...
            await e.reply_text(f"**porn Raid Resumed In {chat_.title}**")
        except Exception as e:
            await e.reply_text(f"**ERROR** \n`{e}`")
    else:
